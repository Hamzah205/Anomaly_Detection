...
<script>document.documentElement.setAttribute("data-theme",localStorage.getItem("pdam_theme")||"light");</script>
  ...
  <style>
    ...
    .sum3{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:24px}
    @media(max-width:600px){.sum3{grid-template-columns:1fr}}
...
async function load() {
  ...
  try {
    ...
    if (historyData) {
      ...
      d = await (await fetch('api.php?action=get_result')).json();
    }
