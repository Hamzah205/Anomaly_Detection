...
def compute_zscore_causes(row, group_stats):
    ...
    gs = group_stats[key]
    context = 'golongan & tahun' if len(key) == 2 else (
        'golongan' if key[0] not in ('ALL', 'MULTI_ALL') else 'global'
    )
...
def analyze(filepath, contamination='0.05', mode='near_tahun_per_golongan',
    ...
    # ── Mode 2: Near Tahun vs Near Golongan (no grouping) ──
    elif mode == 'near_tahun_near_golongan':
        for attr in ['Rp', 'M3', 'Rp_per_M3']:
            if ('ALL',) not in group_stats: group_stats[('ALL',)] = {}
            group_stats[('ALL',)][attr] = {'mean': df[attr].mean(), 'std': df[attr].std()}
...
def analyze_json(json_file_path='stdin', contamination='0.05', mode='near_tahun_per_golongan',
    ...
    # ── Mode 3 & 4: Multi Tahun ──
        ...
        if mode == 'multi_tahun_semua_golongan':
            key = ('MULTI_ALL',)
            group_stats[key] = {}
            for attr in ['Rp', 'M3', 'Rp_per_M3']:
