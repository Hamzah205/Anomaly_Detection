...
<div style="max-width:780px;margin:0 auto">
  ...
  <!-- Hero -->
  <div class="hero">
    <img src="assets/logo.jpg" class="hero-logo" alt="Logo PDAM">
    <div class="hero-text">
...
<label class="opt"><input type="radio" name="mode" value="multi_tahun_per_golongan">
  ...
  <div style="display:flex;gap:12px;justify-content:flex-end;flex-wrap:wrap" class="btn-row">
    <button class="btn-ghost" onclick="location.href='dashboard2.php'">
...
async function checkSys() {
  try {
    const d = await (await fetch('api.php?action=check')).json();
    ...
    if (!d.python) {
      ...
      alert2(`Hasil analisis sebelumnya tersedia.
        <a href="dashboard2.php" style="font-weight:700;background:var(--primary);color:#fff;padding:3px 12px;border-radius:var(--radius-sm);text-decoration:none;margin-left:6px">Buka Dashboard 2 &rarr;</a>`,'success');
...
async function installDeps() {
  ...
  try {
    const d = await (await fetch('api.php?action=install_deps')).json();
    Loading.hide();
...
async function runAnalysis() {
  ...
  try {
    ...
    alert2(`<strong>Analisis selesai!</strong> Ditemukan
      ...
      <a href="dashboard2.php" style="margin-left:8px;background:var(--primary);color:#fff;padding:4px 14px;border-radius:var(--radius-sm);text-decoration:none;font-weight:700;font-size:12px">Lihat Hasil &rarr;</a>`,
      'success');
  } catch(e) {
