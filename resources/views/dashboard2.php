...
<?php include __DIR__ . '/head_common.php'; ?>
  ...
  <style>
    ...
    @media(max-width:480px){.stat-grid{grid-template-columns:repeat(4,1fr);gap:6px}}
    ...
    .chart2{display:grid;grid-template-columns:280px 1fr;gap:16px;margin-bottom:20px}
    @media(max-width:720px){.chart2{grid-template-columns:1fr 1fr;gap:10px}}
    ...
    /* ===== Global Filter Card: grid responsif & hierarki rapi ===== */
    .gfilter-card .gfilter-grid {
      display:grid;
      grid-template-columns:repeat(3,minmax(0,1fr));
      gap:12px;
      align-items:end;
    }
    .gfilter-card .filter-group { display:flex; flex-direction:column; gap:5px; min-width:0; }
    .gfilter-card .filter-group label { font-size:10px; font-weight:700; color:var(--text-2); text-transform:uppercase; letter-spacing:.6px; }
    .gfilter-card .filter-group select {
      width:100%; height:40px; padding:0 11px; min-width:0;
      border:1.5px solid var(--border); border-radius:var(--radius-sm);
      background:var(--bg); color:var(--text-1); font-size:13px; font-family:'DM Sans',sans-serif;
      outline:none; transition:border-color .15s,box-shadow .15s;
    }
    .gfilter-card .filter-group select:focus { border-color:var(--primary); box-shadow:0 0 0 3px rgba(99,102,241,.14); }
    .gfilter-card .btn-run {
      grid-column:1 / -1;
      width:100%; height:42px; padding:0 18px; border-radius:var(--radius-sm);
      display:inline-flex; align-items:center; gap:6px; justify-content:center;
      background:var(--primary); color:#fff; border:none; font-size:13px; font-weight:700;
      cursor:pointer; font-family:'DM Sans',sans-serif; white-space:nowrap;
      box-shadow:0 2px 8px rgba(99,102,241,.35); transition:all .15s;
    }
    .gfilter-card .btn-run:hover { background:var(--primary-dark); transform:translateY(-1px); box-shadow:0 4px 14px rgba(99,102,241,.45); }
    .gfilter-card .mode-tabs { gap:6px; margin-bottom:0; }
    @media (max-width:768px) {
      .gfilter-card .filter-group select { height:40px; font-size:12px; }
    }
    @media (max-width:480px) {
      .gfilter-card .gfilter-grid { grid-template-columns:1fr 1fr; }
      .gfilter-card .filter-group:first-child { grid-column:1 / -1; grid-row:2; max-width:none; width:100%; }
      .gfilter-card .filter-group select { height:42px; font-size:12px; }
    }

    /* ===== Chart cards: typography & layout khusus MOBILE ===== */
    @media (max-width:768px) {
      .chart2 .card-pdam .section-title { font-size:12.5px; }
      .donut-legend { gap:12px; margin-top:10px; }
      .dleg { font-size:11px; }
      .dleg-dot { width:9px; height:9px; }
    }
    @media (max-width:560px) {
      .chart2 { grid-template-columns:1fr; gap:12px; }
      .chart2 .card-pdam { padding:16px 14px; }
      .donut-wrap > div:first-child { width:min(220px,100%) !important; height:auto !important; aspect-ratio:1; }
      #donutCenter #dcPct { font-size:20px !important; }
      #donutCenter > div:last-child { font-size:9px !important; }
    }
</style>
</head>
<body>
...
<div class="main-content">
  ...
  <!-- Global Filter Card -->
  <div class="card-pdam gfilter-card" style="margin-bottom:24px">
...
async function loadResult() {
  try {
    const d = await (await fetch('<?= url('/api') ?>?action=get_result')).json();
...
function render(data) {
  ...
  cBar = new Chart(document.getElementById('cBar').getContext('2d'), {
    ...
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{position:'top',labels:{font:{size:mobile?10:11,family:'DM Sans'},boxWidth:mobile?10:12}},
        tooltip:{
          callbacks: mobile ? {
            title:(items)=>{ const g=gs[items[0].dataIndex]; return `Golongan: ${g?.golongan||''}`; },
            label:(ctx)=>`${ctx.dataset.label}: ${ctx.parsed.y.toLocaleString('id-ID')}`,
            afterBody:(items)=>{ const g=gs[items[0].dataIndex]; if(!g) return [];
              const pct=g.total>0?(g.anomali/g.total*100):0;
              return [`Total: ${g.total.toLocaleString('id-ID')}`,`Persentase Anomali: ${pct.toFixed(1)}%`]; }
          } : undefined
        }},
      scales:{x:{grid:{display:false},ticks:{font:{size:11,family:'DM Sans'},display:!mobile}},
              y:{grid:{color:'rgba(128,128,128,.08)'},ticks:{font:{size:10,family:'DM Sans'},callback:v=>v.toLocaleString('id-ID')}}}}
  });
...
// Re-render chart saat breakpoint mobile/desktop berubah
