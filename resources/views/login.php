...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($login) || empty($password)) {
        ...
        if ($result['ok']) {
            $redirect = $_GET['redirect'] ?? '/dashboard1';
            $allowed  = ['/','/dashboard1','/dashboard2','/dashboard3','/dashboard4','/history'];
            if (!in_array($redirect, $allowed)) $redirect = '/dashboard1';
            header('Location: ' . url($redirect)); exit;
        } else {
...
<div class="login-wrap">
  <div class="login-card">
    ...
    <div class="login-logo">
      <img src="<?= url('/public/assets/logo.jpg') ?>" alt="Logo PDAM" class="login-logo-img">
    ...
    <button class="btn-guest" onclick="location.href='<?= url('/dashboard1') ?>'">
