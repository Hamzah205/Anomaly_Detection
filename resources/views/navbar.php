...
$__req_uri    = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH) ?? '/';
if (defined('BASE_URL') && BASE_URL !== '' && strpos($__req_uri, BASE_URL) === 0) {
    $__req_uri = substr($__req_uri, strlen(BASE_URL));
}
$current_route = '/' . trim($__req_uri, '/');
$current_route = $current_route === '/' ? '/' : rtrim($current_route, '/');
$current      = $current_route;

$pages = [
  '/dashboard1' => ['label'=>'Upload & Parameter',
    'icon'=>'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
  '/dashboard2' => ['label'=>'Global Analysis',
    'icon'=>'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z'],
  '/dashboard3' => ['label'=>'Visual Analytics',
    'icon'=>'M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z'],
  '/dashboard4' => ['label'=>'Detail Anomali',
...
<nav class="navbar-pdam">
  ...
  <a href="<?= url('/dashboard1') ?>" class="navbar-brand">
    <img src="<?= url('/public/assets/logo.jpg') ?>" alt="Logo PDAM" class="navbar-logo">
  ...
  <ul class="navbar-nav-items">
    <li>
      <a href="<?= url('/') ?>" class="<?= $current === '/' ? 'active' : '' ?>">
    ...
    <li>
      <a href="<?= url($file) ?>" class="<?= $current === $file ? 'active' : '' ?>">
        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path d="<?= $info['icon'] ?>"/>
        </svg>
        <?= $info['label'] ?>
  ...
  <div class="nb-user" id="nbUser">
    ...
    <div class="nb-drop" id="nbDrop">
      ...
      <a href="<?= url('/history') ?>" class="nb-drop-item">
        <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
        </svg>
        Riwayat Analisis
      </a>
      <a href="<?= url('/logout') ?>" class="nb-drop-item nb-drop-danger">
        <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
  ...
  <?php else: ?>
  <a href="<?= url('/login') ?>" class="nb-login-btn">
...
<div class="sidebar-drawer" id="sidebarDrawer">
  ...
  <div style="margin-bottom:8px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,.1);display:flex;align-items:center;gap:10px">
    <img src="<?= url('/public/assets/logo.jpg') ?>" alt="Logo" style="height:28px;border-radius:6px">
  ...
  <?php else: ?>
  <a href="<?= url('/login') ?>" style="background:rgba(255,255,255,.1);border-radius:8px;padding:9px 12px;margin-bottom:8px;color:#fff;text-decoration:none;font-size:12px;font-weight:600;display:flex;align-items:center;gap:8px;border:1px solid rgba(255,255,255,.12)">
    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      <path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4"/>
      <polyline points="10 17 15 12 10 7"/>
      <line x1="15" y1="12" x2="3" y2="12"/>
  ...
  <div style="margin-top:auto;padding-top:12px;border-top:1px solid rgba(255,255,255,.1)">
    <a href="<?= url('/logout') ?>" style="color:rgba(255,110,110,.9);display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;text-decoration:none;padding:8px 12px;border-radius:8px;transition:background .2s"
