...
<?php include __DIR__ . '/head_common.php'; ?>
  ...
  <style>
    .sum4{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}
    @media(max-width:900px){.sum4{grid-template-columns:repeat(4,1fr);gap:8px}}
    @media(max-width:480px){.sum4{grid-template-columns:repeat(4,1fr);gap:6px}}
    ...
    @media(max-width:700px){.chart-row{grid-template-columns:1fr 1fr;gap:10px}}
...
<div class="main-content">
  ...
  <div class="card-pdam" style="max-width:520px;margin:40px auto;text-align:center;padding:48px 32px">
    ...
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
      <a href="<?= url('/login') ?>" style="background:var(--primary);color:#fff;padding:10px 24px;border-radius:9px;text-decoration:none;font-weight:700;font-size:13px">
        Login Sekarang
      </a>
      <a href="<?= url('/register') ?>" style="background:var(--bg);color:var(--text-1);border:1.5px solid var(--border);padding:10px 24px;border-radius:9px;text-decoration:none;font-weight:700;font-size:13px">
        Daftar Akun
      </a>
      <a href="<?= url('/dashboard1') ?>" style="background:var(--bg);color:var(--text-2);border:1.5px solid var(--border);padding:10px 24px;border-radius:9px;text-decoration:none;font-weight:600;font-size:13px">
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:20px">
    ...
    <a href="<?= url('/dashboard1') ?>" style="display:inline-flex;align-items:center;gap:7px;background:var(--primary);color:#fff;padding:9px 20px;border-radius:9px;text-decoration:none;font-weight:700;font-size:13px">
  ...
  <div class="card-pdam">
    <div class="empty-state">
      ...
      <p>Riwayat akan muncul setelah Anda menjalankan analisis pertama.<br>
         Mulai dari <a href="<?= url('/dashboard1') ?>" style="color:var(--primary);font-weight:700">Dashboard 1</a>.</p>
    </div>
  </div>
  <?php else: ?>
...
function replayAnalysis(mode, cont, tMin, tMax, historyId) {
  if (historyId) {
    fetch('<?= url('/api') ?>?action=load_history', {
      method: 'POST',
    ...
    .then(text => {
      ...
      if (d.status === 'success' && d.data) {
        ...
        location.href = (window.BASE_URL || '') + '/dashboard4?from_history=1&history_id=' + historyId;
      } else {
    ...
    location.href = (window.BASE_URL || '') + '/dashboard2?replay=1';
  }
}
