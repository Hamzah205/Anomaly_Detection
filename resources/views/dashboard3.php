...
<?php include __DIR__ . '/head_common.php'; ?>
  ...
  <style>
    /* ===== Filter Bar: grid seimbang & tinggi seragam ===== */
    .filter-bar { gap:10px 12px; }
    .filter-group { flex:1 1 150px; min-width:135px; }
    .filter-group select,
    ...
    .filter-bar .btn-run,
    .filter-bar .btn-export-excel,
    .filter-bar .btn-export-pdf {
      flex:0 0 auto; height:40px; padding:0 18px; margin:0;
    }

    /* ===== Chart Cards: dua kartu proporsional ===== */
    .chart-2col { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:20px; align-items:stretch; }
    .chart-2col .card-pdam { margin-bottom:0; display:flex; flex-direction:column; }
    .chart-2col .card-pdam > div:last-child { flex:1; min-height:0; }
    ...
    @media (max-width: 768px) {
      .filter-bar { gap:8px 10px; padding:12px; }
      .filter-group { flex:1 1 44%; min-width:120px; max-width:none; }
      .filter-group select,
      .filter-group input[type="number"],
      .filter-group input[type="text"] { height:38px; padding:0 9px; font-size:12px; }
      .filter-bar .btn-run,
      .filter-bar .btn-export-excel,
      .filter-bar .btn-export-pdf { flex:1 1 auto; height:38px; padding:0 12px; font-size:11.5px; justify-content:center; }
    }
    @media (max-width: 640px) {
      .chart-2col { grid-template-columns:1fr; gap:12px; }
      .filter-group { flex:1 1 46%; min-width:110px; }
      .filter-bar .btn-run { flex:1 1 100%; }
      .filter-bar .btn-export-excel,
      .filter-bar .btn-export-pdf { flex:1 1 calc(50% - 5px); }
    }
